...
circom_pipeline_build_circuit() {
    ...
    if circom_pipeline_needs_recompile "$@"; then
        ...
        echo "compiling Circom wrapper into $CIRCUIT_BUILD_DIR" >&2
        circom --prime "$GROTH16_CURVE" --r1cs --wasm --sym --output "$CIRCUIT_BUILD_DIR" "$WRAPPER_CIRCUIT"
        circom_pipeline_write_build_config
        touch "$COMPILE_STAMP"
    fi
...
circom_pipeline_run_e2e_test() {
    local work_dir="${1:-$CIRCUIT_BUILD_DIR/groth16_sample_proof}"
    local aiken_vk_output_path="${2:-}"

    circom_pipeline_ensure_tool cargo

    "$BUILD_CIRCUIT_SCRIPT" >/dev/null
