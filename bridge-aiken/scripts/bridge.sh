...
Official short path:
  ...
  uv sync
  # Compile the Circom circuits once (and whenever a .circom source changes);
  # the bridge flow proves against circuit_build/, which is not generated by
  # `run`. Requires the `circom` binary on PATH.
